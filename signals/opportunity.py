...
class Leg:
    ...
    metric: float
    qty: float
    expected_deltaent_node_id: str
    expected_deltaent_node_title: str = ""
...
class Opportunity:
    ...
    expected_deltaent_id: Optional[str] = None
    ...
    @property
    def expected_deltaent_node_ids(self) -> list[str]:
        return list({leg.expected_deltaent_node_id for leg in self.legs})

    def to_dict(self) -> dict[str, Any]:
        return {
            "engine": self.engine, "kind": self.kind,
            "detected_at": self.detected_at.isoformat(),
            "expected_deltaent_id": self.expected_deltaent_id,
            "expected_deltaent_node_ids": self.expected_deltaent_node_ids,
            "efficiency_delta_pct": self.efficiency_delta_pct,
...
class OpportunityQueue:
    ...
    async def put(self, opp: Opportunity) -> None:
        self._counter += 1
        priority = ENGINE_PRIORITY.get(opp.engine, 99)
